...
describe("resolveAndValidateConfigExports", () => {
	it("selects bindings from the Preview build context", async ({ expect }) => {
		const worker = defineWorker(({ isPreview, mode }) => ({
			name: `${isPreview ? "preview" : "production"}-${mode}`,
			compatibilityDate,
			env: {
				TARGET: bindings.text(isPreview ? "preview" : "production"),
			},
		}));
		...
		const production = await resolveAndValidateConfigExports(
			{ default: worker },
		...
		expect(production.success && production.data.default).toMatchObject({
			name: "production-staging",
			env: { TARGET: { value: "production" } },
		});
	});
